...
export default function RecallSequenceAnalytics() {
  ...
  const loadData = async () => {
    try {
      ...
      // Recall Sequence results
      const recallResults = Array.isArray(analytics?.recallSequence)
        ? analytics.recallSequence
        : [];

      const patientResults = recallResults.filter(
      ...
      console.error("Recall Sequence analytics error:", err);
  ...
  const sequenceLengths = results
    .map((result) => Number(result?.sequenceLength))
    .filter((value) => Number.isFinite(value));

  const totalAttempts = results
    .map((result) => Number(result?.totalAttempts))
    .filter((value) => Number.isFinite(value));

  const correctAttempts = results
    .map((result) => Number(result?.correctAttempts))
    .filter((value) => Number.isFinite(value));

  const mistakes = results
    .map((result) => Number(result?.mistakes))
    .filter((value) => Number.isFinite(value));

  const responseTimes = results
    .map((result) => Number(result?.responseTime))
    .filter((value) => Number.isFinite(value));
  ...
  const avgSequenceLength = average(sequenceLengths);

  const avgAttempts = average(totalAttempts);
    ...
    return (
      <div className="analytics-page">
        <div className="analytics-loading">
          Loading Recall Sequence analytics...
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="analytics-page">
      ...
      <div className="analytics-stats">
        ...
        <div className="analytics-stat-card">
          <span>Avg Sequence</span>

          <strong>
            {sequenceLengths.length ? avgSequenceLength.toFixed(1) : "—"}
          </strong>
        </div>
        ...
        <section className="analytics-section">
          ...
          <div className="latest-performance">
            ...
            <div>
              ...
              <strong>{latestResult?.sequenceLength ?? "—"}</strong>
            </div>
      ...
      <section className="analytics-section">
        ...
        {results.length === 0 ? (
          <div className="empty-state">
            ...
            <h3>No Recall Sequence sessions yet</h3>

            <p>
              Once the patient plays Recall Sequence, their performance will
              appear here.
            </p>
          </div>
        ) : (
